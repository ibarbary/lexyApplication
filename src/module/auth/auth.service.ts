...
import { SignupDto,ConfirmEmailDto,loginDto, ForgetPasswordDto, ResetPasswordDto} from "./auth.dto";
import { BadRequestException, NotFoundException } from "../../utils/errors/error.response";
...
const pending_user = await this.UserModel_pending.findone({filter:{email}});
if(!pending_user){
...
return res.status(200).json({message:"user loged in success",Credentials });
...
await this._UserModel.updateOne({filter:{email},update:{forgetPasswordOtp:await hashtext(String(otp))}})

emailEvent.emit("forgotPassword", { to: email, username:user.username , otp });
...
const {email,password,otp,confirmPassword}:ResetPasswordDto = req.body ;
...
await this._UserModel.updateOne({filter:{email},update:{password:await hashtext(password)}})
